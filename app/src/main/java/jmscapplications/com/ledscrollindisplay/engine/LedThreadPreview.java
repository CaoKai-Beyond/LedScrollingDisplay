...
public class LedThreadPreview extends Thread {
    private static final float PREDEFINE_HEIGHT = 100.0f;
    private final int CHARACTER_HEIGHT_RESOLUTION = 18;
    ...
    public LedThreadPreview(SurfaceHolder holder, Context ctx, LedParameters parameters) {
    ...
    private void configureLedMessage() {
        ...
        this.radius = (int) (((((float) this.surfaceHeight) / 18.0f) / 2.0f) * 0.95f);
        if (this.parameters.isDirectionStraight()) {
    ...
    private int[][] getTextMatrix(String text) {
        ...
        this.TEXT_WIDHT_RESOLUTION = (int) (18.0f * (((float) w) / PREDEFINE_HEIGHT));
        this.OUTPUT_SCREEN_WIDHT_RESOLUTION = (int) ((18.0f * (((float) this.surfaceWidth) * factorScreenSize)) / PREDEFINE_HEIGHT);
        int[][] outputTextMatrix = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.TEXT_WIDHT_RESOLUTION, 18});
        int xSearchDistance = (int) (((float) w) / ((float) this.TEXT_WIDHT_RESOLUTION));
        int ySearchDistance = (int) (((float) 100) / 18.0f);
